...
-- Learners retain their existing read-only entitlement policy. Admin writes go
-- through the checked SECURITY DEFINER functions below.
...
create or replace function public.admin_list_lessons(p_course_id uuid)
returns table (
  id uuid,
  course_id uuid,
  title text,
...
begin
  ...
  return query
    select l.id, l.course_id, l.title, l.slug, l.description, l.video_url,
      l.position, l.is_published, l.created_at, l.updated_at
...
set search_path = public
as $$
declare target public.lessons%rowtype; neighbor public.lessons%rowtype;
begin
  if not public.is_admin() then raise exception 'Administrator access required'; end if;
  ...
  perform public.normalize_lesson_positions(target.course_id);
  return query select l.id, l.position from public.lessons l where l.id = target.id;
end;
$$;
...
set search_path = public
as $$
declare target public.lessons%rowtype;
begin
  if not public.is_admin() then raise exception 'Administrator access required'; end if;
  select l.* into target from public.lessons as l where l.id = p_lesson_id for update;
  if not found then raise exception 'Lesson not found'; end if;
