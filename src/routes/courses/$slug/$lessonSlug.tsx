...
export const Route = createFileRoute("/courses/$slug/$lessonSlug")({
  component: () => (
    <AuthenticatedRouteGuard>
      <AdminPremiumLessonAccess />
    </AuthenticatedRouteGuard>
  ),
});

function AdminPremiumLessonAccess() {
  const { slug } = Route.useParams();
  const { isAdmin, loading } = useAdmin();

  if (loading)
    return <div className="py-24 text-center text-sm text-muted-foreground">Verifying access…</div>;
  if (!isAdmin) return <Navigate to="/courses/$slug" params={{ slug }} replace />;
  return <PremiumLesson />;
}
