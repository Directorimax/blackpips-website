...
function Blog() {
  ...
  const filtered = useMemo(
    () =>
      BLOG_POSTS.filter(
        (p) =>
          (cat === "All" || p.category === cat) && p.title.toLowerCase().includes(q.toLowerCase()),
      ),
    [q, cat],
  );
    ...
    <div className="mx-auto max-w-7xl px-4 py-16">
      <header className="mx-auto max-w-3xl text-center">
        ...
        <h1 className="mt-4 font-display text-4xl font-bold sm:text-5xl">
          Essays for serious traders
        </h1>
        <p className="mt-3 text-muted-foreground">
          Deep dives on structure, liquidity, psychology and the mechanics of institutional flow.
        </p>
      </header>
        ...
        <div className="glass flex flex-wrap gap-1 rounded-full p-1">
          {CATS.map((c) => (
            <button
              key={c}
              onClick={() => setCat(c)}
              className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${cat === c ? "bg-gradient-gold text-primary-foreground shadow-glow" : "text-muted-foreground hover:text-foreground"}`}
            >
              {c}
            </button>
          ))}
          ...
          >
            <div className="relative aspect-[16/10] overflow-hidden bg-gradient-to-br from-accent to-secondary">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-gradient-gold font-display text-4xl font-black opacity-40">
                  {p.category
                    .split(" ")
                    .map((w) => w[0])
                    .join("")}
            ...
            <div className="flex flex-1 flex-col p-6">
              ...
              <div className="mt-4 border-t border-border pt-3 text-xs text-muted-foreground">
                {new Date(p.date ?? "2025-04-01").toLocaleDateString(undefined, {
                  month: "short",
                  day: "numeric",
                  year: "numeric",
                })}
