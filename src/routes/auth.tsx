...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Sign in or create a BlackPips account to access premium lessons, bookmarks and progress tracking." },
...
function AuthPage() {
  ...
  const sub = mode === "signup" ? "Start learning the ALC framework in minutes." : mode === "forgot" ? "We'll email you a secure link to set a new password." : "Sign in to continue where you left off.";
    ...
    <div className="mx-auto flex min-h-[calc(100vh-8rem)] max-w-md items-center px-4 py-16">
      <div className="glass w-full rounded-3xl p-8 shadow-elegant">
        <div className="text-center">
          <div className="inline-flex rounded-full border border-gold/30 bg-gold/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-gold">BlackPips</div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          {mode === "signup" && (
            <label className="block">
              <span className="mb-1 block text-xs font-semibold text-muted-foreground">Full name</span>
              <input
                value={name}
        ...
        <div className="mt-5 flex flex-col items-center gap-1 text-xs text-muted-foreground">
          {mode === "signin" && (
            <>
              ...
              <div>
                New here?{" "}
                <button onClick={() => setMode("signup")} className="font-semibold text-gold hover:underline">Create an account</button>
              </div>
            </>
          )}
          {mode === "signup" && (
            <div>
              ...
              <button onClick={() => setMode("signin")} className="font-semibold text-gold hover:underline">Sign in</button>
            </div>
          )}
          {mode === "forgot" && (
...
function GoogleIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 48 48" aria-hidden="true">
      ...
      <path fill="#4CAF50" d="M24 44c5.4 0 10.3-2 14-5.4l-6.5-5.5c-2 1.4-4.6 2.3-7.5 2.3-5.3 0-9.7-3.4-11.3-8l-6.5 5C9.5 39.6 16.2 44 24 44z"/>
      <path fill="#1976D2" d="M43.6 20.5H42V20H24v8h11.3c-.8 2.2-2.2 4.1-4 5.5l6.5 5.5c4-3.7 6.5-9.2 6.5-15.5 0-1.2-.1-2.4-.4-3.5z"/>
    </svg>
  );
}
